...
sliderRange.onchange = function() {
    ...
    if(selected=='landuse'){
      ...
      document.getElementById("map").style.display = "none";
      document.getElementsByClassName('legend')[0].style.display = 'none'; 
      document.getElementById('pop_diff').style.display = 'block';
      document.getElementsByClassName('legend_com_pop')[0].style.display = 'block';
      document.getElementsByClassName('legend_ml')[0].style.display = 'none';
      document.getElementById("comparison-container").style.display = "none";
      curr_map=population;
    }
...
for (const input of inputs) {
  ...
  input.onclick = (layer) => {
    ...
    map_on_screen=selected_button;
    if(selected_button=='landuse'||selected_button=='income'||selected_button=='education'||selected_button=='transit'||selected_button=='noise'||selected_button=='green'){
        // Switch between maps
      document.getElementById("map").style.display = "block"; 
      document.getElementsByClassName('legend')[0].style.display = 'block';
      document.getElementById("housing_diff").style.display = "none";
      document.getElementsByClassName('legend_com')[0].style.display = 'none';
      document.getElementById("pop_diff").style.display = "none";
      document.getElementsByClassName('legend_com_pop')[0].style.display = 'none';
      document.getElementById("comparison-container").style.display = "none";
      document.getElementsByClassName('legend_ml')[0].style.display = 'none';
    }
    else if(selected_button=='population'){//  Population Comaprison
        console.log('Compare_Popupaltion');
        document.getElementById("map").style.display = "none";
        document.getElementsByClassName('legend')[0].style.display = 'none';
        document.getElementById("housing_diff").style.display = "none";
        document.getElementsByClassName('legend_com')[0].style.display = 'none';
        ...
        if(checking_pop ==0){
          ...
          const container = '#pop_diff';  
          const map_com = new mapboxgl.Compare(map, afterMap, container, {
          });
          ...
          map.on('click',function(event){// click arguement+listener funciton
            ...
            if(features.length){                           //automatically convert expressions inside the () to a Boolean value, if the number is 0, it would be convert to false --> features.length>0   
                ...
                var population = lot.properties.population || '—' ;
          ...
          afterMap.on('click',function(event){// click arguement+listener funciton
            ...
            if(features_2.length){                           //automatically convert expressions inside the () to a Boolean value, if the number is 0, it would be convert to false --> features.length>0   
             ...
             }); //if(features_2.length)     
        }//if statement   
    }// else if
  
    else {    //  Housing Comaprison
        console.log('Compare_Housing');
        document.getElementById("map").style.display = "none"; 
        document.getElementsByClassName('legend')[0].style.display = 'none';
        document.getElementById("housing_diff").style.display = "block";
        document.getElementsByClassName('legend_com')[0].style.display = 'block';
        ...
        if(checking_housing==0){
          ...
          map.on('click',function(event){// click arguement+listener funciton
                ...
                if(features.length){                           //automatically convert expressions inside the () to a Boolean value, if the number is 0, it would be convert to false --> features.length>0   
                    ...
                    var borough = lot.properties.borough || '—';
                    var zipcode = lot.properties.zipcode || '—';
                    var housing_units = lot.properties.housing_units || '—' ;
                    var unitsres = lot.properties.unitsres || '—';
...
map.on('click',function(event){// click arguement+listener funciton
  ...
  var features = map.queryRenderedFeatures(geometry, parameters);
  var lot = features[0];// get all the features in each tax lot
  // console.log('lot.properties.landuse',lot.properties.landuse);
  console.log('map_on_screen',map_on_screen)
  if(map_on_screen=='landuse'){
    console.log('currently showing',map_on_screen)
    if(features.length){                           //automatically convert expressions inside the () to a Boolean value, if the number is 0, it would be convert to false --> features.length>0   
        var bbl = lot.properties.bbl || '—'; // if the first value is undefined or empty, will get the second value
        var zipcode = lot.properties.zipcode || '—';
        var landuse = landUseNames[lot.properties.landuse] || '—' ;
      ...
      else if(map_on_screen=='income'){
        console.log('currently showing',map_on_screen)
          if(features.length){                           //automatically convert expressions inside the () to a Boolean value, if the number is 0, it would be convert to false --> features.length>0   
          var bbl = lot.properties.bbl || '—'; // if the first value is undefined or empty, will get the second value
          var zipcode = lot.properties.zipcode || '—';
          var income_pc = lot.properties.income_pc || '—' ;
          var med_income = lot.properties.med_income || '—' ;
      ...
      else if(map_on_screen=='education'){
        console.log('currently showing',map_on_screen)
        if(features.length){                           //automatically convert expressions inside the () to a Boolean value, if the number is 0, it would be convert to false --> features.length>0   
          var bbl = lot.properties.bbl || '—'; // if the first value is undefined or empty, will get the second value
          var zipcode = lot.properties.zipcode || '—';
          var low_edu = lot.properties.low_edu || '—' ;
          //var low_edu_percentage = lot.properties.%low_educ	 || '—' ;
      ...
      else if(map_on_screen=='transit'){
        console.log('currently showing',map_on_screen)
        if(features.length){                           //automatically convert expressions inside the () to a Boolean value, if the number is 0, it would be convert to false --> features.length>0   
          var bbl = lot.properties.bbl || '—'; // if the first value is undefined or empty, will get the second value
          var borough = lot.properties.borough || '—';
          var zipcode = lot.properties.zipcode || '—';
          var subway_acc = lot.properties.subway_acc || '—' ;
          var subway = lot.properties.subway	 || '—' ;
      ...
      else if(map_on_screen=='green'){
        if(features.length){                           //automatically convert expressions inside the () to a Boolean value, if the number is 0, it would be convert to false --> features.length>0   
          var bbl = lot.properties.bbl || '—'; // if the first value is undefined or empty, will get the second value
          var borough = lot.properties.borough || '—';
          var zipcode = lot.properties.zipcode || '—';
          var built_green = lot.properties.built_green || '—' ;
      ...
      else if(map_on_screen=='noise'){
        if(features.length){                           //automatically convert expressions inside the () to a Boolean value, if the number is 0, it would be convert to false --> features.length>0   
          var bbl = lot.properties.bbl || '—'; // if the first value is undefined or empty, will get the second value
          var borough = lot.properties.borough || '—';
          var zipcode = lot.properties.zipcode || '—';
          var noisePerCapita = lot.properties.noisePerCapita || '—' ;
          ...
          var popup = new mapboxgl.Popup()
              ...
              '<dd>'+  noisePerCapita +'</dd>'+
              '</dl>')
              .addTo(map);
           }
      }

      else if(map_on_screen=='housing'){
        if(features.length){                           //automatically convert expressions inside the () to a Boolean value, if the number is 0, it would be convert to false --> features.length>0   
          var bbl = lot.properties.bbl || '—'; // if the first value is undefined or empty, will get the second value
          var borough = lot.properties.borough || '—';
          var zipcode = lot.properties.zipcode || '—';
